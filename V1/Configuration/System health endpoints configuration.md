...
```bash
curl -v -d "@System_HealthEndpoints.json" -H "Content-Type: application/json" -X POST http://localhost:5590/api/v1/configuration/System/HealthEndpoints
```
...
| **Endpoint**                                                    | Optional  | `string` | Yes      | The URL of the ingress point which accepts OMF health messages.|
